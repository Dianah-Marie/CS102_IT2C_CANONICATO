...
url <- 'https://arxiv.org/search/?query=Philosophy&searchtype=all&abstracts=show&order=-announced_date_first&size=50'
...
# Export the result
save(papers, file = "Arvix_Philosophy.RData")
write.csv(papers, file = "ArxivPapers_Philosophy.csv")
...
articles <- read.csv("ArxivPapers_Philosophy.csv")
tail(articles)
...
#dbWriteTable(connection,'arvixTable', articles, append = TRUE)

```

```{}
dbListTables(connection)
dbListFields(connection,'arvixTable')
```

```{}

review_data <- dbGetQuery(connection, "SELECT * FROM canonicato_2c.arvixTable")
glimpse(review_data)
